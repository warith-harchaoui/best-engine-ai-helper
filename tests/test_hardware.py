...
def test_load_hardware_and_lookup_chip() -> None:
    ...
    assert (
        hardware.lookup_chip("apple m2 max", entries)["chip"]
        == hardware.lookup_chip("APPLE M2 MAX", entries)["chip"]
    )
    # Multi-tier chips (M2 Max at 32/64/96 GB) return the FIRST list match (32 GB).
    assert float(hardware.lookup_chip("Apple M2 Max", entries)["memory_gb"]) == 32.0
    # A known discrete GPU resolves with its VRAM.
    rtx = hardware.lookup_chip("RTX 4090", entries)
    assert rtx["vendor"] == "nvidia" and float(rtx["memory_gb"]) == 24.0
    # An unknown chip is a miss, not an error.
    assert hardware.lookup_chip("NonExistent GPU 9000", entries) is None


def test_detect_local_entry_and_write_cache(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        detect, "available_memory", lambda: {"unified_gb": 32.0, "vram_gb": None, "ram_gb": 32.0}
    )
    apple_entry = hardware.detect_local_entry(fetched_at="2026-08-01")
    assert apple_entry["chip"] == "Apple M2 Max" and apple_entry["memory_gb"] == 32.0
    assert apple_entry["ollama_usable_gb"] == 28.0 and apple_entry["source"] == "detected"
    ...
    monkeypatch.setattr(
        detect, "available_memory", lambda: {"unified_gb": None, "vram_gb": 24.0, "ram_gb": 64.0}
    )
    gpu_entry = hardware.detect_local_entry(fetched_at="2026-08-01")
    assert "NVIDIA" in gpu_entry["chip"] and gpu_entry["memory_gb"] == 24.0
    ...
    monkeypatch.setattr(
        detect, "available_memory", lambda: {"unified_gb": None, "vram_gb": None, "ram_gb": 0.0}
    )
